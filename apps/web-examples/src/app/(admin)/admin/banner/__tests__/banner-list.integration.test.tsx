...
import AdminBannerListPage from '../page';
...
describe('배너 목록 페이지', () => {
  ...
  it("제목 '배너 관리'가 렌더링 된다.", () => {
    renderPage();

    expect(screen.getByRole('heading', { name: '배너 관리' })).toBeInTheDocument();
  });
  ...
  describe('테이블', () => {
    ...
    it("테이블은 '체크박스', '배너명', '노출 상태', '순서', '작성일자', '관리' 컬럼을 갖는다.", () => {
      const table = screen.getByRole('table');
      expect(table).toBeInTheDocument();

      const tableHeader = screen.getAllByRole('columnheader');
      expect(within(tableHeader[0]).getByRole('checkbox')).toBeInTheDocument();
      expect(screen.getByRole('columnheader', { name: '배너명' })).toBeInTheDocument();
    ...
    it('각 행의 관리 버튼은 수정 페이지의 링크이다.', async () => {
      await waitFor(() => {
        ...
        expect(managementLink).toHaveAttribute('href', '/admin/banner/1/edit');
      });
    });
  });

  describe('페이지 사이즈 (전체 데이터 개수 11개)', () => {
  ...
  it('추가하기 버튼은 배너 추가 페이지로 이동하는 링크를 가진다.', () => {
    renderPage();
    ...
    // href 속성 확인
    expect(addLink).toHaveAttribute('href', '/admin/banner/create');
  });
  ...
  describe('삭제', () => {
    ...
    describe('삭제 모달', () => {
      beforeEach(async () => {
        ...
        // 모달 렌더링 대기
        expect(await screen.findByText('선택된 배너를 삭제하시겠습니까?')).toBeInTheDocument();
      });

      it('"선택된 배너를 삭제하시겠습니까?", 닫기 버튼, 삭제 버튼이 있는 모달이 표시된다.', async () => {
        expect(screen.getByText('선택된 배너를 삭제하시겠습니까?')).toBeInTheDocument();
      ...
      it('닫기 버튼을 클릭하면 모달이 닫힌다.', async () => {
        ...
        await waitFor(() => {
          expect(screen.queryByText('선택된 배너를 삭제하시겠습니까?')).not.toBeInTheDocument();
        });
      });

      it('삭제 버튼을 클릭하면 삭제 후 모달이 닫히며, 삭제된 행은 테이블 목록에서 제거된다.', async () => {
        // 삭제 전 데이터 존재 확인
        expect(screen.getByText('테스트 배너 1')).toBeInTheDocument();
        ...
        await waitFor(() => {
          expect(screen.queryByText('테스트 배너 1')).not.toBeInTheDocument();
        });
  ...
  describe('필터', () => {
    beforeEach(async () => {
      ...
      expect(totalCount).toHaveTextContent('총 11개');
    ...
    it('필터 적용 시 데이터의 총 개수 및 검색된 데이터의 개수가 변경된다.', async () => {
      const { totalCount, searchCount } = await getTotalAndSearchCount();
      expect(totalCount).toHaveTextContent('총 11개');
      expect(searchCount).toHaveTextContent('검색 5개');
    });

    it('필터 적용 시 필터링 된 데이터가 테이블에 표시된다.', async () => {
      expect(await screen.findByText('테스트 배너 1')).toBeInTheDocument();
      expect(screen.queryByText('테스트 배너 11')).not.toBeInTheDocument();
    });
  ...
  describe('검색', () => {
    ...
    it('검색 시 필터링 된 데이터가 테이블에 표시된다.', async () => {
      const [banner1, banner2, banner3] = await Promise.all([
        screen.findByText('테스트 배너 1'),
        screen.findByText('테스트 배너 11'),
        screen.findByText('테스트 배너 10'),
      ]);
  ...
  it('초기화 버튼 클릭 시 필터 및 검색이 초기화된다. (setup.ts의 useResetSearchFilter 모킹 참고)', async () => {
    render(
      <TestProviders searchParams={{ isShow: 'true', searchType: 'title', keyword: '1' }}>
        <AdminBannerListPage />
      </TestProviders>
    );
    ...
    expect(totalCount2).toHaveTextContent('총 11개');
