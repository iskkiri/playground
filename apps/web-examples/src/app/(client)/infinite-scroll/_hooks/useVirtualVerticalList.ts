import { virtualVerticalListScrollInfoAtom } from '@/app/(client)/infinite-scroll/_atoms/scroll.atom';
import type { MockUser } from '@/_features/user/api/dtos/getUser.dto';
import { useWindowVirtualizer } from '@tanstack/react-virtual';
import { useAtom } from 'jotai';
import { useRef } from 'react';

interface UseVirtualVerticalListParams {
  flatList: MockUser[];
}

export default function useVirtualVerticalList({ flatList }: UseVirtualVerticalListParams) {
  const listRef = useRef<HTMLDivElement>(null);
  // 스크롤 복원
  const [scrollCache, setScrollCache] = useAtom(virtualVerticalListScrollInfoAtom);
